...
class Pipeline:
    def __init__(self):
        ...
        # Prompts
        self.schema_matching_prompt_template = """You are an intelligent assistant. Based on the following database schema, check where this user's question belongs or is related to this schema. Respond strictly with yes or no without any explanations or additional details.
        Schema:{schema}
        Question: {question}
        Match:"""
        self.schema_matching_prompt = ChatPromptTemplate.from_template(self.schema_matching_prompt_template)

        self.generate_sql_prompt_template = """Generate only the SQL query to answer the user's question. Do not include any explanations, natural language responses, or other text:
        {schema}
        Question: {question}
        SQL Query:"""
        self.generate_sql_prompt = ChatPromptTemplate.from_template(self.generate_sql_prompt_template)

        self.visualization_prompt_template = """Output the following data directly as a clean table without any introductory text, explanations, or additional information:
        {query_result}
        """
        self.visualization_prompt = ChatPromptTemplate.from_template(self.visualization_prompt_template)
    ...
    def pipe(self, user_message: str, model_id: str, messages: List[dict], body: dict) -> Union[str, Generator, Iterator]:
        ...
        return f"Generated SQL Query: {generated_sql}\n\nQuery Results:\n{formatted_output}"
